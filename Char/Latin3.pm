...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin3::uc($char[$i]) ne Char::Elatin3::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin3::uc($char[$i]) . Char::Elatin3::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin3/Char::Latin3/JA.pm
...
Definitions in Char/Elatin3.pm.
  ...
  @{Char::Elatin3::eV}             qr{(?:[^\x0C\x0A\x0D])}
